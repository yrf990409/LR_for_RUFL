                   ...
                   const coder::array<real_T, 2U> &cus_mu)
{
  ...
  if (r.size(1) <= R + 1.0) {
    //  没有到达设施数量的上界
    if (r.size(1) < 1) {
      emlrtDynamicBoundsCheckR2012b(r.size(1), 1, r.size(1), &cc_emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
    if (r[r.size(1) - 1] == 1.0) {
      //  如果结尾是虚拟，那么获得了一个完整路径
      if (cur_cost < *ub) {
        ...
        //  小于上界 则更新
        *ub = cur_cost;
        best_r.set_size(&fc_emlrtRTEI, sp, 1, r.size(1));
        loop_ub = r.size(1);
      ...
      i = r.size(1);
      r.set_size(&wb_emlrtRTEI, sp, r.size(0), r.size(1) + 1);
      r[i] = 1.0;
      //  route必须是虚拟收尾了
      ...
      if (tempCost < *ub) {
        ...
        //  小于上界 则更新
        *ub = tempCost;
        best_r.set_size(&ec_emlrtRTEI, sp, 1, r.size(1));
        loop_ub = r.size(1);
        for (i = 0; i < loop_ub; i++) {
          best_r[i] = r[i];
        }
      ...
      if (tempCost < *ub) {
        ...
        //  小于上界 则更新
        *ub = tempCost;
        best_r.set_size(&dc_emlrtRTEI, sp, 1, r.size(1));
        loop_ub = r.size(1);
        for (i = 0; i < loop_ub; i++) {
          best_r[i] = r[i];
        }
      ...
      for (int32_T b_i{0}; b_i < i; b_i++) {
        ...
        stackR.set_size(&vb_emlrtRTEI, sp, 1, r.size(1));
        loop_ub = r.size(1);
        ...
        r.set_size(&wb_emlrtRTEI, sp, r.size(0), r.size(1) + 1);
        if (b_i + 1 > fac.size(1)) {
          emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, fac.size(1), &dc_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        r[i1] = fac[b_i];
        if (b_i + 1 > fac.size(1)) {
          emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, fac.size(1), &ec_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        i1 = static_cast<int32_T>(fac[b_i]);
        if ((i1 < 1) || (i1 > cus_mu.size(1))) {
          emlrtDynamicBoundsCheckR2012b(i1, 1, cus_mu.size(1), &bc_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        ...
        if ((static_cast<int32_T>(d) < 1) ||
          ...
          emlrtDynamicBoundsCheckR2012b(static_cast<int32_T>(d), 1,
                                        cost_mat.size(0), &fc_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        if (r.size(1) < 1) {
          emlrtDynamicBoundsCheckR2012b(r.size(1), 1, r.size(1), &ic_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        b_fac = r[r.size(1) - 1];
        ...
        if (tempCost > *ub) {
          //  剪枝
          r.set_size(&cc_emlrtRTEI, sp, 1, stackR.size(1));
          ...
          //  递归 继续分支
          tempFacy.set_size(&xb_emlrtRTEI, sp, 1, fac.size(1));
          loop_ub = fac.size(1);
          for (i1 = 0; i1 < loop_ub; i1++) {
            tempFacy[i1] = fac[i1];
          }
          if (b_i + 1 > fac.size(1)) {
            emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, fac.size(1), &jc_emlrtBCI,
                                          (emlrtConstCTX)sp);
          }
          b_fac = fac[b_i];
          c_fac.set_size(&yb_emlrtRTEI, sp, 1, fac.size(1));
          loop_ub = fac.size(1);
          for (i1 = 0; i1 < loop_ub; i1++) {
          ...
          if (r.size(1) < 1) {
            emlrtDynamicBoundsCheckR2012b(r.size(1), 1, r.size(1), &lc_emlrtBCI,
                                          (emlrtConstCTX)sp);
          }
          d = r[r.size(1) - 1];
          if (d != static_cast<int32_T>(muDoubleScalarFloor(d))) {
          ...
          if ((static_cast<int32_T>(d) < 1) ||
            ...
            emlrtDynamicBoundsCheckR2012b(static_cast<int32_T>(d), 1,
                                          probDisr.size(0), &kc_emlrtBCI,
                                          (emlrtConstCTX)sp);
          }
          cur_prob *= probDisr[static_cast<int32_T>(d) - 1];
          b_r.set_size(&ac_emlrtRTEI, sp, 1, r.size(1));
          ...
          r.set_size(&bc_emlrtRTEI, sp, 1, stackR.size(1));
          loop_ub = stackR.size(1);
          for (i1 = 0; i1 < loop_ub; i1++) {
            r[i1] = stackR[i1];
          }
  ...
  if (R + 1.0 >= 1.0) {
    ...
    if (r_data[0] == 1.0) {
      ...
      if (*ub > 0.0) {
        ...
        //  小于上界 则更新
        *ub = 0.0;
        best_r.set_size(&fc_emlrtRTEI, sp, 1, r_size[1]);
        loop_ub = r_size[1];
        for (int32_T i{0}; i < loop_ub; i++) {
          best_r[i] = r_data[i];
        }
      ...
      if (tempCost < *ub) {
        ...
        //  小于上界 则更新
        *ub = tempCost;
        best_r.set_size(&ec_emlrtRTEI, sp, 1, r_size[1]);
        loop_ub = r_size[1];
        for (i = 0; i < loop_ub; i++) {
          best_r[i] = r_data[i];
        }
      ...
      if (tempCost < *ub) {
        ...
        //  小于上界 则更新
        *ub = tempCost;
        best_r.set_size(&dc_emlrtRTEI, sp, 1, r_size[1]);
        loop_ub = r_size[1];
        for (i = 0; i < loop_ub; i++) {
          best_r[i] = r_data[i];
        }
      ...
      for (int32_T b_i{0}; b_i < i; b_i++) {
        ...
        i2 = static_cast<int32_T>(fac[b_i]);
        if ((i2 < 1) || (i2 > cus_mu.size(1))) {
          emlrtDynamicBoundsCheckR2012b(i2, 1, cus_mu.size(1), &bc_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        if (r_data[0] != static_cast<int32_T>(muDoubleScalarFloor(r_data[0]))) {
          emlrtIntegerCheckR2012b(r_data[0], &q_emlrtDCI, (emlrtConstCTX)sp);
        }
        i2 = static_cast<int32_T>(r_data[0]);
        if ((i2 < 1) || (i2 > cost_mat.size(0))) {
          emlrtDynamicBoundsCheckR2012b(i2, 1, cost_mat.size(0), &fc_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        i3 = static_cast<int32_T>(muDoubleScalarFloor(r_data[1]));
        if (r_data[1] != i3) {
          emlrtIntegerCheckR2012b(r_data[1], &r_emlrtDCI, (emlrtConstCTX)sp);
        }
        i4 = static_cast<int32_T>(r_data[1]);
        if ((i4 < 1) || (i4 > cost_mat.size(1))) {
          emlrtDynamicBoundsCheckR2012b(i4, 1, cost_mat.size(1), &hc_emlrtBCI,
                                        (emlrtConstCTX)sp);
        }
        ...
        if (tempCost > *ub) {
          ...
          coder::internal::nullAssignment(&st, tempFacy, c_fac);
          if (r_data[1] != i3) {
            emlrtIntegerCheckR2012b(r_data[1], &s_emlrtDCI, (emlrtConstCTX)sp);
          }
          if (i4 > probDisr.size(0)) {
            emlrtDynamicBoundsCheckR2012b(i4, 1, probDisr.size(0), &kc_emlrtBCI,
                                          (emlrtConstCTX)sp);
          }
          r.set_size(&ac_emlrtRTEI, sp, 1, i1);
